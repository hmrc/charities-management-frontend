...
class JsErrorOpsSpec extends BaseSpec {
  ...
  "JsErrorOps.prettyPrint" - {
    ...
    "format a single path with a single error message" in {
      ...
      error.prettyPrint() shouldBe "/fieldName: [error.required]"
    }
    ...
    "format a single path with multiple error messages" in {
      ...
      error.prettyPrint() shouldBe "/fieldName: [error.required,error.invalid]"
    }
    ...
    "format multiple paths separated by semicolons" in {
      ...
      result should include("/field1: [error.required]")
      result should include("/field2: [error.invalid]")
      result should include("; ")
    }

    "format a nested path" in {
      val error = JsError((JsPath \ "outer" \ "inner") -> JsonValidationError("error.missing"))
      error.prettyPrint() shouldBe "/outer/inner: [error.missing]"
    }
